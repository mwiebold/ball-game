...
| Audio | **Web Audio API** | Synthesized notes (no audio assets), melody sequencing, hooks into MediaRecorder for export with sound. |
| Recording | **MediaRecorder** (`canvas.captureStream()` + audio destination node) | Native WebM export, no server. |
...
│  └─ events.ts         # BOUNCE, RING_BREAK, PICKUP, WIN, LOSE
├─ render/
...
├─ audio/
│  ├─ synth.ts          # WebAudio voices (sine/square/pluck)
│  └─ melody.ts         # note sequences; BOUNCE event → next note
├─ config/
...
│  └─ recorder.ts       # MediaRecorder wiring
...
- `core/` never touches DOM/Canvas/Audio — it consumes settings + seed and
  emits an event stream. This is what makes runs deterministic (F-7),
  re-recordable (F-12), and testable (N-8).
...
particle bursts with budget-based degradation (N-2); Web Audio synth; melody
mode with bundled sequences; ring-break SFX; caption + timer HUD styling to
match the reference.
**Done when:** side-by-side with the source video, a "Reference video" preset
run is a convincing match; audio behind user-gesture gate.

### Phase 4 — Capture & share (v1.0)
MediaRecorder WebM export (canvas + audio); PNG snapshot; user presets in
localStorage; preset JSON import/export; mobile drawer layout & a11y pass;
GitHub Pages deploy.
**Done when:** one click yields a WebM clip of a full run with sound; app
usable on a phone; deployed URL live.

### Phase 5 — Stretch (P3 backlog, post-review)
Multiple balls + ball-ball collisions and pickups (F-8/F-9 — may pull into
Phase 2 if prioritized), alternative destruction modes, fixed-fps re-render
export, custom melody import, aspect-ratio options, screen shake.
...
| MediaRecorder quirks across browsers (Safari WebM) | Feature-detect; fall back to MP4 where supported or document Chrome as the export browser. Fixed-fps re-render (F-12) is the long-term fix. |
...
1. **Now** — this plan + requirements (especially REQUIREMENTS §6 open questions).
